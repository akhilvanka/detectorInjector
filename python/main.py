...
def genGPTRes(fileName, userId = None, postId= None):
    ...
    messages=[
        ...
        {"role": "user", "content": f"Please write a responce to this assignment as if you were the student:\n{text}"}
    ]
    )
...
@app.post('/upload')
async def upload(file: UploadFile, toInject: Annotated[str, Form()], userId: Annotated[str, Form()], background_tasks: BackgroundTasks):
    print(toInject)
    fileName = ""
    try:
    ...
    doc_ref.set({"userId": userId, "docUrl": blob.public_url, "createdAt":firestore.SERVER_TIMESTAMP, "GPTresponse":"Loading"})
